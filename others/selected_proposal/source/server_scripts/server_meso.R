  ...
  # observeEvent(input$keep.m,{
  #   #input$crop!="-- Please select crop --" && 
  #   if(nchar(input$nombre.m)!=0 ){
  #     updateTabsetPanel(session, "tabset1.m",
  #                       selected = "Occurrence")
  #     
  ...
  output$mymap4.m<-renderLeaflet({
    leaflet("mymap4.m") %>%
      ...
      addMarkers(lng=presence$Longitude[which(presence$Genepool.predicted=="Mesoamerican")],lat=presence$Latitude[which(presence$Genepool.predicted=="Mesoamerican")]  ,icon=beanIcon,label=(as.character(presence$Genepool.predicted[which(presence$Genepool.predicted=="Mesoamerican")])),clusterOptions=markerClusterOptions(spiderfyOnMaxZoom= F,
  ...
  EVT_DRAW0 <- "mymap4.m_draw_new_feature"
  EVT_EDIT0<- "mymap4.m_draw_edited_features"
  EVT_DELETE0 <- "mymap4.m_draw_deleted_features"
  ...
  EVT_DRAW <- "mymap5.m_draw_new_feature"
  EVT_EDIT <- "mymap5.m_draw_edited_features"
  EVT_DELETE <- "mymap5.m_draw_deleted_features"
  ...
  output$plot1.m<-renderLeaflet({
    ...
    if(length(returnlist.m()$finished)!=0){
      ...
      leaflet("plot1.m") %>%
        ...
        )%>% addPolygons(data=returnlist.m()$finished, color = "blue", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                         opacity = 0.5, fillOpacity = 0.5) 
      
    }else{leaflet("plot1.m") %>%
        addProviderTiles(providers$Esri.WorldGrayCanvas, #map type or map theme. -default($Stamen.TonerLite)
                         options = providerTileOptions(noWrap = TRUE) 
    ...
    isolate({
      ...
      if(length(returnlist.m()$finished)!=0){
        ...
        leaflet("mymap6.m") %>%
          ...
          ) %>%  addPolygons(data=returnlist.m()$finished,color = "darkgray", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
  ...
  observeEvent(input$mymap5.m_draw_new_feature,{
    
    proxy<-leafletProxy("mymap6.m")
    if(length(returnlist.m()$finished)!=0){
      proxy %>%  clearShapes()%>% addScaleBar(position="bottomleft")%>%  addSearchOSM() %>% addPolygons(data=harvest, group="Harvested area" ,stroke = T,color = "#f7ba94", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                                                                                                        opacity = 0.5, fillOpacity = 0.5,
                                                                                                        fillColor = "#dd9e87") %>%  addLayersControl(overlayGroups="Harvested area",options=layersControlOptions(collapsed=F)) %>%addPolygons(data = returnlist.m()$finished,
  ...
  observeEvent(input$mymap5.m_draw_edited_features,{
    proxy<-leafletProxy("mymap6.m")
    if(length(returnlist.m()$finished)!=0){
      ...
      proxy %>%  clearShapes()%>% addScaleBar(position="bottomleft")%>%  addSearchOSM( ) %>% addPolygons(data=harvest, group="Harvested area" ,stroke = T,color = "#f7ba94", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                                                                                                         opacity = 0.5, fillOpacity = 0.5,
                                                                                                         fillColor = "#dd9e87") %>%  addLayersControl(overlayGroups="Harvested area",options=layersControlOptions(collapsed=F)) %>% addPolygons(data = returnlist.m()$finished,color = "darkgray", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                                                                                                                                                                                                                                                opacity = 0.5, fillOpacity = 0.05,
                                                                                                                                                                                                                                                fillColor = colorQuantile("Blues", domain=NULL)
                                                                                                         )
  ...
  observeEvent(input$mymap5.m_draw_deleted_features,{
    proxy<-leafletProxy("mymap6.m")
    if(length(returnlist.m()$finished)!=0){
  ...
  EVT_DRAW2 <- "mymap6.m_draw_new_feature"
  EVT_EDIT2 <- "mymap6.m_draw_edited_features"
  EVT_DELETE2 <- "mymap6.m_draw_deleted_features"
  ...
  observeEvent(input$next2.m,{
    updateTabsetPanel(session, "tabset1.m",
                      selected = "3. Collecting")
    
  })
  observeEvent(input$back2.m,{
  ...
  output$plot2.m<-renderLeaflet({
    ...
    if(length(returnlist.m2()$finished)!=0){
      ...
      leaflet("plot2.m") %>%
        ...
        )%>% addPolygons(data=returnlist.m2()$finished, color = "red", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                         opacity = 0.5, fillOpacity = 0.5) 
      
    }else{leaflet("plot2.m") %>%
        addProviderTiles(providers$Esri.WorldGrayCanvas, #map type or map theme. -default($Stamen.TonerLite)
                         options = providerTileOptions(noWrap = TRUE) 
    ...
    isolate({
      if(length(returnlist.m2()$finished)!=0){
        ...
        leaflet("mymap7.m") %>%
          ...
          )%>% addPolygons(data=returnlist.m2()$finished,color = "darkgray", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                           opacity = 0.5, fillOpacity = 0.05,
                           fillColor = colorQuantile("Blues", domain=NULL)) %>% addScaleBar(position="bottomleft")%>%  addSearchOSM() %>% addPolygons(data=harvest, group="Harvested area" ,stroke = T,color = "#f7ba94", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                                                                                                                                                      opacity = 0.5, fillOpacity = 0.5,
                                                                                                                                                      fillColor = "#dd9e87") %>%  addLayersControl(overlayGroups="Harvested area",options=layersControlOptions(collapsed=F))   %>% 
  ...
  observeEvent(input$mymap6.m_draw_new_feature,{
    
    proxy<-leafletProxy("mymap7.m")
    if(length(returnlist.m2()$finished)!=0){
      
      
      proxy %>%  clearShapes() %>% addScaleBar(position="bottomleft")%>%  addSearchOSM( ) %>% addPolygons(data=harvest, group="Harvested area" ,stroke = T,color = "#f7ba94", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                                                                                                          opacity = 0.5, fillOpacity = 0.5,
                                                                                                          fillColor = "#dd9e87") %>%  addLayersControl(overlayGroups="Harvested area",options=layersControlOptions(collapsed=F))   %>% addPolygons(data = returnlist.m2()$finished,
                                                                                                                                                                                                                                                   color = "darkgray", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                                                                                                                                                                                                                                                   opacity = 0.5, fillOpacity = 0.05,
                                                                                                                                                                                                                                                   fillColor = colorQuantile("Blues", domain=NULL)
                                                                                                          )
  ...
  observeEvent(input$mymap6.m_draw_edited_features,{
    ...
    if(length(returnlist.m2()$finished)!=0){
      ...
      proxy %>%  clearShapes() %>% addScaleBar(position="bottomleft")%>%  addSearchOSM( ) %>% addPolygons(data=harvest, group="Harvested area" ,stroke = T,color = "#f7ba94", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                                                                                                          opacity = 0.5, fillOpacity = 0.5,
                                                                                                          fillColor = "#dd9e87") %>%  addLayersControl(overlayGroups="Harvested area",options=layersControlOptions(collapsed=F))  %>% addPolygons(data = returnlist.m2()$finished,color = "darkgray", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                                                                                                                                                                                                                                                  opacity = 0.5, fillOpacity = 0.05,
                                                                                                                                                                                                                                                  fillColor = colorQuantile("Blues", domain=NULL)
                                                                                                          )
  ...
  observeEvent(input$mymap6.m_draw_deleted_features,{
  ...
  EVT_DRAW3 <- "mymap7.m_draw_new_feature"
  EVT_EDIT3 <- "mymap7.m_draw_edited_features"
  EVT_DELETE3 <- "mymap7.m_draw_deleted_features"
  ...
  observeEvent(input$next3.m,{
    updateTabsetPanel(session, "tabset1.m",
                      selected = "4. Already conserved")
    
  })
  observeEvent(input$back3.m,{
  ...
  output$plot3.m<-renderLeaflet({
    ...
    if(length(returnlist.m3()$finished)!=0){
      ...
      leaflet("plot3.m") %>%
        ...
        )%>% addPolygons(data=returnlist.m3()$finished, color = "orange", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                         opacity = 0.5, fillOpacity = 0.5) 
      
    }else{leaflet("plot3.m") %>%
        addProviderTiles(providers$Esri.WorldGrayCanvas, #map type or map theme. -default($Stamen.TonerLite)
                         options = providerTileOptions(noWrap = TRUE) 
  ...
  EVT_DRAW4 <- "mymap8.m_draw_new_feature"
  EVT_EDIT4 <- "mymap8.m_draw_edited_features"
  EVT_DELETE4 <- "mymap8.m_draw_deleted_features"
  ...
  observeEvent(input$next4.m,{
    updateTabsetPanel(session, "tabset1.m",
                      selected = "Results")
    
  })
  observeEvent(input$back4.m,{
  ...
  observeEvent(input$close.m,{
    
    
    # if(length(returnlist.m()$finished)!=0) savePlots(returnlist.m()$finished,gsub(" ","",input$nombre),countries2,col="blue",n.quest=1,gsub(" ", "",input$menu))
    # if(length(returnlist.m2()$finished)!=0)savePlots(returnlist.m2()$finished,gsub(" ","",input$nombre),countries2,col="red",n.quest=2,gsub(" ", "",input$menu))
    # if(length(returnlist.m3()$finished)!=0) savePlots(returnlist.m3()$finished,gsub(" ","",input$nombre),countries2,col="green",n.quest=3,gsub(" ", "",input$menu))
    # if(length(returnlist.m4()$finished)!=0)savePlots(returnlist.m4()$finished,gsub(" ","",input$nombre),countries2,col="orange",n.quest=4,gsub(" ", "",input$menu))
  ...
  output$plot4.m<-renderLeaflet({
    ...
    if(length(returnlist.m4()$finished)!=0){
      ...
      leaflet("plot4.m") %>%
        ...
        )%>% addPolygons(data=returnlist.m4()$finished, color = "green", weight = 1, smoothFactor = 0.2,  #adicionar el archivo .shp al mapa y hacer que brillen cuadno son seleccionados
                         opacity = 0.5, fillOpacity = 0.5) 
      
    }else{leaflet("plot4.m") %>%
        addProviderTiles(providers$Esri.WorldGrayCanvas, #map type or map theme. -default($Stamen.TonerLite)
                         options = providerTileOptions(noWrap = TRUE) 

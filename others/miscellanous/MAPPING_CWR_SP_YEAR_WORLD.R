...
#
GRIN2<-GRIN
GEO_TO_FIX<-strsplit(GRIN2$Geography,",")
GEO_TO_FIX<-unlist(lapply(GEO_TO_FIX,function(x){
  
  x[[1]]
    })
  )

GEO_TO_FIX<-as.data.frame(as.character(GEO_TO_FIX))
GEO_TO_FIX$ISO3<-NA
colnames(GEO_TO_FIX)<-c("GEOGRAPHY","ISO3")
for(i in 1:nrow(GAUL_TABLE)){
  cat(i,"\n")
  GEO_TO_FIX$ISO3[which(GEO_TO_FIX$GEOGRAPHY==as.character(GAUL_TABLE$Short.name[[i]]))]<-as.character(GAUL_TABLE$ISO3[[i]])
}


GRIN2$Geography<-GEO_TO_FIX$GEOGRAPHY
GRIN2$ISO3_DS<-NA
GRIN2$ISO3_DS<-GEO_TO_FIX$ISO3
...
GRIN4$ISO_MATCH<-NA
for(i in 1:nrow(GRIN4)){
  cat(i,"\n")

  
  if(is.na(GRIN4$ISO3_OVR[[i]])){
    
    GRIN4$ISO_MATCH[[i]]<-0
    
  }else if(is.na(GRIN4$ISO3_DS[[i]])){
    GRIN4$ISO_MATCH[[i]]<-0
    
  }else if(GRIN4$ISO3_DS[[i]]==GRIN4$ISO3_OVR[[i]]){
  
  GRIN4$ISO_MATCH[[i]]<-1
}else if(is.na(GRIN4$ISO3_DS[[i]]) & is.na(GRIN4$ISO3_OVR[[i]])){
  GRIN4$ISO_MATCH[[i]]<-2
...
GG_TO_FIX<-(GRIN4[which(GRIN4$ISO_MATCH==0),])
write.csv(GG_TO_FIX,paste0(out_dir,"/WORLD/","GRIN_TO_FIX_",Sys.Date(),".csv"),quote=F,row.names=F,na="")
############################################
##--GBIF--#
...
for(i in 1:length(years)){
  ...
  map1<-ggplot() + 
    ...
    coord_fixed(1.3)+
    geom_point(aes(x=LONGITUDE, y=LATITUDE,col=SOURCE),size=1.2,stroke = 1,data=x, show.legend = TRUE,shape=17)+#Alt + 9733  
    coord_equal() 
  ...
  map1<-map1+ theme(axis.text=element_text(size=10),axis.title=element_text(size=10),legend.position="bottom",legend.title=element_blank())#+
    ...
    ggsave(paste0(out_dir,"/WORLD/",years[[i]],"_",Sys.Date(),".pdf"),units="in",width=8,height=4.8,scale=2,dpi=600)
...
# mask<-raster("U:/Input_data/mask_wb_c_ant.tif")
# mask2<-mask
# mask2[]<-1:ncell(mask2)
...
#   
# }
write.csv(data_final,paste0("U:/Input_data/_occurrence_data/_GBIF_data_2017_11_27/WORLD/DATA_CLEANED_ONLY_YEAR.csv"),row.names=F,quote=F,na="")
write.csv(data_final2,paste0("U:/Input_data/_occurrence_data/_GBIF_data_2017_11_27/WORLD/DATA_CLEANED_ALL_YEAR.csv"),row.names=F,quote=F,na="")
write.csv(data_final3,paste0("U:/Input_data/_occurrence_data/_GBIF_data_2017_11_27/WORLD/DATA_CLEANED_GBIF_YEAR.csv"),row.names=F,quote=F,na="")
write.csv(data_final4,paste0("U:/Input_data/_occurrence_data/_GBIF_data_2017_11_27/WORLD/DATA_CLEANED_GBIF_YEAR2000.csv"),row.names=F,quote=F,na="")
...
for(i in 1:length(dat_fin_list)){
  ...
  map1<-map1+ theme(axis.text=element_text(size=10),axis.title=element_text(size=10),legend.position="bottom",legend.title=element_blank())#+
   ...
   ggsave(paste0(out_dir,"/WORLD/",labs[[i]],"_",Sys.Date(),".pdf"),units="in",width=8,height=4.8,scale=2,dpi=600)
}

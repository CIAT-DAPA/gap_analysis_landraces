...
pacman::p_load(raster,sp, exactextractr, sf, tidyverse,lattice, rasterVis, 
               ...
               collapsibleTree, stringi, writexl, biscale, cowplot, stars,  RCurl, XML, lobstr, sftp)
...
if(TRUE){
  ...
  msk <- raster(paste0(baseDir,"/input_data/mask/mask_world.tif"))
...
#save consolidado S4 class in a RData file
save(consolidado, file = paste0(baseDir, "/results/results_all_crops.RData"))

#load("D:/LGA_dashboard/www/results_all_crops.RData")

#####################################################
############## update results in FTP server ########
####################################################
...
for(i in 1:nrow(consolidado@crop)){
 ...
 ## Gap richness map per crop

for(i in 1:nrow(consolidado@crop)){
  var<- consolidado@crop %>% 
    dplyr::slice(i) %>% 
    dplyr::select(pth, name) 
 cat("Copying gap richness map to FTP server for: ", unlist(var$name), "\n")
  
 gp_rich <- list.files(path = unlist(var$pth), pattern = "gap_richness_[a-zA-Z0-9_-]+.tif$", full.names = T)
  
 if(length(gp_rich) != 0){
   r <- raster::raster(gp_rich)
   
   output_pth <- paste0(root_ftp, "/spatial_lga/", unlist(var$name), "/results")
   if(dir.exists(output_pth)){
     raster::writeRaster(r, paste0(output_pth, "/gap_richness_",  unlist(var$name), ".tif"  ), overwrite =T)
   }else{
     cat("Directory not found for:", unlist(var$name), "\n")
   }
...
rsdm_msk <- raster::mask(rsdm, rsin_r) 
perct_r <- (rsin_r/rsdm_msk)*100

#tmp_file <-  "D:/perct.tif"
#writeRaster(perct_r, tmp_file)

rsin <- perct_r#raster("Z:/gap_analysis_landraces/runs/results/all_crops_gap_richness.tif")
wrld <- raster::shapefile('Z:/gap_analysis_landraces/runs/input_data/shapefiles/shp/all_countries.shp')
ssdn <- raster::shapefile('Z:/gap_analysis_landraces/runs/input_data/shapefiles/Sudan_admin_WGS84/Sudan_admin_0.shp')
msk <- raster("Z:/gap_analysis_landraces/runs/input_data/mask/mask_world.tif")
grat <- sp::gridlines(wrld, easts = seq(-180, 180, by = 20), norths = seq(-90, 90, by = 20))
...
mt <- matrix(c(0, 33, 1, 
               33, 66, 2,
               66, 100, 3), 3,3, byrow = T)

rsin      <- raster::reclassify(x = rsin, rcl = mt, include.lowest=F)
rsin      <- raster::ratify(rsin)
rat       <- levels(rsin)[[1]]
rat$level <- c("0%-33%", "34%-66%", "> 66%")
if(nrow(rat) == 1){
  cols <- '#2991c6'
}else{
  cols <- grDevices::colorRampPalette(c('#2991c6', '#fafa65', '#e61614'))(nrow(rat))
...
hght = 3.5
wdth = 10
png("Z:/gap_analysis_landraces/runs/results/all_crops_gap_percentage.png", height = hght, width = wdth, units = "in", res = 300)
print(p)
dev.off()
...
#####################################
### COnvirtiendo rasters en tablas##
####################################

msk <- raster("Z:/gap_analysis_landraces/runs/input_data/mask/mask_world.tif")

pths <- gap_maps_pths %>%
  dplyr::filter(!(paths %in% "Not_found")) %>% 
  dplyr::pull(paths)

dfs <- lapply(pths, function(i){
  cat("Retrieving data from:", i, "\n")
  r  <- tryCatch(expr = {
    r <- raster(i)
    r <- raster::extend(r, extent(msk))
  }, error = function(e){r <- NA})
  if("Raster" %in% is(r)){
    df <- as.data.frame(r, xy = T)
    if(ncell(msk) != nrow(df)){stop("number of rows does not agree")}
    rm(r);removeTmpFiles(h = 0)
    df$ID <- 1:nrow(df)
    df <- df[, -c(1,2)]
  }else{df  <- data.frame(ID= NA, var = NA)}
  return(df)
}) %>% purrr::reduce(.,  left_join,  by = c("ID"))

saveRDS(dfs, "Z:/gap_analysis_landraces/runs/input_data/dfs.rds")

########  trabajar  raster en formato de tablas (No muy buena idea)
...
crops_paths <- data.frame(pth = paste0(list.dirs(path = root_dir, full.names = T, recursive = F), "/lvl_1") ,
                          ...
                          stringsAsFactors = F)
tic()
dfs <- apply(crops_paths, 1,  function(i){
  cat("Retrieving data for: ",  unlist(i[2]), '\n')
  rich_pth <- paste0(i[1],  "/gap_richness_", i[2],  ".tif")
  
  if(file.exists(rich_pth)){
    r <- raster::raster(rich_pth)
    r <- raster::extend(r, extent(msk)) 
    df <- as.data.frame(r, xy = T)
    if(ncell(msk) != nrow(df)){stop("number of rows does not agree")}
    rm(r);removeTmpFiles(h = 0)
...
toc()
dfs <- readRDS("Z:/gap_analysis_landraces/runs/input_data/dfs.rds")

##### Funcion para descargar los datos desde el servidor SFTP
...
gp_rich <- getBinaryURL(url = file_url)
...
http <- "https://www.google.com/maps/api/directions/json?origin=place_id:ChIJ8bNLzPCmMI4RaGGuUum1Dx8&destination=place_id:ChIJKcumLf2bP44RFDmjIFVjnSM&travelmode=driving&key=AIzaSyD0eNlbzDQp0IUKTIOyyvnYW5_uFDdA7bw"
 ...
 coords <- jsonlite::fromJSON(text, flatten =F , simplifyMatrix = F, simplifyVector = F)

 x <- coords$routes[[1]]$legs[[1]]$steps[[1]] %>% 
   bind_rows()
 x[1,]
pl1<- x$polyline[[1]] %>% decode_pl()
 
 x2 <- coords$routes[[1]]$legs[[1]]$steps[[2]] %>% 
   bind_rows()
 
 pl2 <- x2$polyline[[1]] %>%  decode_pl()
 
 to_pl <- bind_rows(pl1, pl2) 
 
 pl <-  data.frame(polyline  = encode_pl(to_pl$lat, to_pl$lon) )
 cocoords$routes
 
 pl <- data.frame(polyline  ="wcaTzsbrMI[Yw@e@aAEI_BaEgAyC")
 
 
 #google_map_update("Gmap") %>% 
 google_map(key = map_key) %>% 
   #add_markers(data = pl) %>% 
   add_polylines(data = pl, polyline = "polyline", stroke_weight = 2) 



pr <- raster(paste0(tempdir(),"/p.tif"))


list(map_key = 'AIzaSyARbwF61yXA-0aEOfeDYanC-IpgfxMQL-w',
     sftp_user = "acmendez",
     sftp_pass = "Mendez2021") %>% 
  saveRDS("D:/LGA_dashboard/www/credential.rds")




#update sftp server with identifiers files

crops_names <- list.dirs("Z:/gap_analysis_landraces/runs/input_data/by_crop", recursive = F, full.names = F)
crops_names <- crops_names[!crops_names %in%  c("wheat_new", "common_bean_onlyCIAT")]

id_paths <- paste0("Z:/gap_analysis_landraces/runs/input_data/by_crop/", crops_names , "/lvl_1/classification/",crops_names, "_bd_identifiers.csv")
id_logical <- file.exists(paste0("Z:/gap_analysis_landraces/runs/input_data/by_crop/", crops_names , "/lvl_1/classification/",crops_names, "_bd_identifiers.csv"))

id_paths <- id_paths[!grepl("old", id_paths)]

df_paths<- data.frame(crops_names, id_paths, id_logical, stringsAsFactors = F)

sftp_crop_names <- list.dirs("Z:/gap_analysis_landraces/shared_results/spatial_lga/", full.names = F, recursive = F)


df_paths$sftp_logical <- crops_names %in% sftp_crop_names

df_paths <- df_paths %>% 
  filter(id_logical & sftp_logical) %>% 
  mutate(sftp_paths = paste0("Z:/gap_analysis_landraces/shared_results/spatial_lga/",crops_names, "/databases/",crops_names, "_used_occ_shp.rds"  )) 
  
msk <- raster("Z:/gap_analysis_landraces/runs/input_data/mask/mask_world.tif")
## pasar archivo de indentifiers al servidor ftp

purrr::map2( .x = df_paths$id_paths, .y = df_paths$sftp_paths, .f = function(.x, .y){
    cat("reading: ", .x, "\n")
  
    occ <- read.csv(.x)
    occ_sp <- occ %>% 
      dplyr::filter(used) %>%
      dplyr::mutate(source_db = tolower(source_db)) %>% 
      dplyr::filter(!grepl("usda|wiews|gbif|genesys|views", source_db)) %>% 
      dplyr::select(Accessibility, Latitude, Longitude) 
      
    coordinates(occ_sp) <- ~Longitude+Latitude
    crs(occ_sp) <- CRS("+proj=longlat +datum=WGS84 +no_defs ")
    r_f <- rasterize(occ_sp, msk, mask = T)
    r_f_pol <- rasterToPolygons(r_f, dissolve = T) 
    saveRDS(r_f_pol, .y)
    
    
  })

gp_rich <- list.files("Z:/gap_analysis_landraces/shared_results/spatial_lga/", pattern = "gap_richness", full.names = T, recursive = T)
names_cr <- str_extract(gp_rich, "//[a-zA-Z0-9_]+") %>% 
  str_extract(., "[a-zA-Z0-9_]+")

paths <- paste0("Z:/gap_analysis_landraces/shared_results/spatial_lga//", names_cr, "/results")

## convertir raster to polygons

future::plan(future::multisession, workers = 10)

furrr::future_pmap( .l = list(i = gp_rich, y = paths, z = names_cr), function(i, y, z){
  cat("reading: ", i, "\n")
  if(file.exists(i)){
    r <- raster(i)
    r[!is.na(r[])] <- 1
    pol <- rasterToPolygons(r, dissolve = T)
    if(dir.exists(y)){
      saveRDS(pol, paste0(y,"/gap_richness_", z, ".rds"))
    }else{
      cat("No dir Found \n")
    }
  }
...
## convertir occurrencias a raster y luego a shapefile
...
### copy files to www folder in shiny app root directory

dirs_names <- list.dirs("Z:/gap_analysis_landraces/shared_results/spatial_lga/", recursive = F, full.names = F)
gp_paths <- list.files("Z:/gap_analysis_landraces/shared_results/spatial_lga/", pattern = "^gap_richness_[a-zA-Z0-9_]+.rds", full.names = T, recursive = T)
bd_paths <- list.files("Z:/gap_analysis_landraces/shared_results/spatial_lga/", pattern = "_used_occ_shp.rds$", full.names = T, recursive = T)
msk <- raster("Z:/gap_analysis_landraces/runs/input_data/mask/mask_world.tif")

future::plan(future::multisession, workers = 10)
furrr::future_map(dirs_names, function(i){
 cat("processing:", i , "\n")
  x  <- paste0("Z:/gap_analysis_landraces/runs/input_data/by_crop/", i, "/lvl_1/classification/",i,"_bd_identifiers.csv") 
  dr <- paste0("Z:/gap_analysis_landraces/others/www/",i)
  ...
  if(file.exists(x)){
    ...
    dest_dir <- dr
    dest_file_shp <- paste0(dest_dir, "/",i, "_used_occ_shp.rds")
    
    saveRDS(r_f_pol, dest_file_shp)

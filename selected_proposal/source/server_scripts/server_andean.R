  ...
  output$mymap4<-renderLeaflet({
    leaflet("mymap4") %>%
      ...
      addMarkers(lng=presence$Longitude[which(presence$Genepool.predicted=="Andean")],lat=presence$Latitude[which(presence$Genepool.predicted=="Andean")],icon=beanIcon,label=(as.character(presence$Genepool.predicted[ which(presence$Genepool.predicted=="Andean")  ])),clusterOptions=markerClusterOptions(spiderfyOnMaxZoom= F,
  ...
  observeEvent(input$done0,{
    ...
    saveData_1(returnlist0()$finished,gsub(" ", "", input$nombre),gsub(" ", "",input$tabset1),(input$done0), gsub(" ", "",input$menu),input$txt0)

...
# write.csv(ciat, file = "C:/Users/haachicanoy/Desktop/Classification/ciat_db.csv", row.names = F)
...
srtm_values <- data.frame(ciat %>% dplyr::filter(!is.na(Latitude) & is.na(Altitude)) %>% dplyr::select(Longitude, Latitude), SRTM = srtm.vals)
...
ciat$Altitude[which(!is.na(ciat$Latitude) & is.na(ciat$Altitude))] <- srtm_values$SRTM
rm(srtm_values)
...
ciat <- ciat %>% dplyr::filter(Altitude <= 3500) # accessions: 15878; 22/03/18: 16031
ciat <- ciat[!is.na(ciat$Longitude),]

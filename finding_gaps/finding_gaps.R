...
cost <- raster("Z:/Input_data/_friction_surface/cost_distances/prueba_cost4OSM.tif")
dgp1 <- raster("//dapadfs/Workspace_cluster_9/gap_analysis_landraces/Input_data/raster_sdm/dist_to_GP1.tif") 
slope <- raster("//dapadfs/Workspace_cluster_9/gap_analysis_landraces/Input_data/raster_sdm/Slope.tif")
sdm_rast <- raster("//dapadfs/Workspace_cluster_9/gap_analysis_landraces/Results/SDM_modelling/Andean/prj_models/Andean_prj_median.tif")
...
r_stk <- raster::stack(sdm_rast, cost_dt, slope_mk, dgp1_mk)
...
save(clust, file = "Z:/Scripts/finding_gaps/clust.RData")

#load("Z:/Scripts/finding_gaps/clust.RData")
...
writeRaster(pres,filename= "Z:/Results/finding_gaps/highprobs_cluster3.tif" , format="GTiff", overwrite = T )
